...
namespace ModIO
{
    ...
    public class UnsubmittedModMedia
    {
        ...
        public string[] imageFilepaths = new string[0];
        ...
        public BinaryDataField[] GetDataFields()
        {
            ...
            if(System.IO.File.Exists(imageZipFilepath))
            {
                BinaryDataField newData = new BinaryDataField();
                newData.key = "images";
                newData.contents = System.IO.File.ReadAllBytes(imageZipFilepath);
                newData.fileName = System.IO.Path.GetFileName(imageZipFilepath);
            ...
            else
            {
                foreach(string imageFilepath in imageFilepaths)
                {
                    BinaryDataField newData = new BinaryDataField();
                    newData.key = "images[]";
                    newData.contents = System.IO.File.ReadAllBytes(imageFilepath);
                    newData.fileName = System.IO.Path.GetFileName(imageFilepath);
